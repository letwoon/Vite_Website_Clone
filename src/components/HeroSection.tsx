...
function HeroSection() {
  ...
  return (
    <div
      ref={ref}
      className="flex flex-col lg:flex-row-reverse lg:w-10/12 justify-end items-center pt-16 lg:pt-36 pb-12"
    >
      <motion.div className="mb-12 relative max-lg:w-full lg:mb-0 flex justify-center lg:justify-center ">
        <motion.img
          style={{ scale }}
          className="w-44 sm:w-48 lg:w-80  ease-in-out"
          src={logo}
          alt="vite logo"
        />
      ...
      <div className="sm:w-4/5 sm:px-8 lg:w-fit lg:mr-32 lg:px-0 lg:text-left container flex flex-col items-center lg:items-start">
        ...
        <h2
          className={`text-4xl sm:text-5xl lg:text-6xl font-bold dark:text-white/90 text-midnight  mt-1 max-w-lg`}
        >
          Next Generation Frontend Tooling
        </h2>
        <p
          className={`text-lg sm:text-xl lg:text-2xl text-midnight/50 dark:text-white/50  lg:text-semibold mt-2 lg:mt-4 max-w-xl`}
        >
        ...
        <div className="mt-3 lg:mt-6 flex gap-5 flex-wrap">
          ...
          <button
            className={`rounded-full dark:bg-[#3A3A3A] dark:text-white/80 dark:hover:brightness-125 dark:border-white/10 dark:active:brightness-90 bg-light_secondary text-midnight hover:brightness-90 border-black/10 active:brightness-75 border  leading-tight px-6 font-semibold text-base py-3   transition-all duration-200`}
          >
            Why Vite?
          </button>
        </div>
